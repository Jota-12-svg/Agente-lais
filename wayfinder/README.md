...
| # | Ticket | Tipo | Espera |
|---|---|---|---|
| 047 | [Detecção de handoff sobrevive a reconexão/boot do runtime](tickets/047-handoff-sobrevive-reconexao.md) | incidente | **grilling e código prontos** (mitigação no `system-prompt.md`, tabela `agent_sent_messages`, backfill via `messaging-history.set` em toda reconexão) — falta aplicar a migration em produção (mesmos bloqueios de permissão do 038/046 podem se repetir) e a **validação ao vivo** (reconexão real com `fromMe` de teste), que depende de alguém escanear o QR de novo — o agente está desconectado desde as 20h43 UTC de hoje |
