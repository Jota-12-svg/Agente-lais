...
| 031 | [Implementar a escrita do chamado do agente na fila (INSERT no Supabase)](tickets/031-implementar-escrita-do-chamado-na-fila.md) | task · desbloqueado (046 fechou) — só falta o item solto `chatModify markRead:false` (035 §5), nunca implementado |
...
>
> **046 item 1 fechado, validado ao vivo em produção** (2026-09-14): esquema de `engagements`
> decidido por grilling (8 perguntas), implementado, migrations rodadas em produção (SQL
> Editor, `db push` por CLI seguiu bloqueado), segredo no Supabase Vault, ligado ao Railway.
> **Validação real**: linha inserida direto no banco, `railway restart`, log confirmou
> reidratação ("restart não perdeu conversa em andamento") — critério de "resolvido" do item 1
> cumprido. Itens 2/3 não tocados.
>
...
| # | Ticket | Tipo | Espera |
|---|---|---|---|
| 047 | [Detecção de handoff sobrevive a reconexão/boot do runtime](tickets/047-handoff-sobrevive-reconexao.md) | incidente | **grilling, código e migration prontos** (mitigação no `system-prompt.md`, tabela `agent_sent_messages` já aplicada em produção — segredo validado com `ERROR 28000: forbidden` de propósito —, backfill via `messaging-history.set` em toda reconexão) — falta só a **validação ao vivo** (reconexão real com `fromMe` de teste), que depende de alguém escanear o QR de novo — o agente está desconectado desde as 20h43 UTC de hoje |
...
| 046 | [Endurecer o runtime do agente — estado, idempotência e deploy automático](tickets/046-endurecer-runtime-estado-idempotencia-deploy.md) | task | fechado 2026-09-14, os três itens: estado (`engagements` no Supabase, restart não perde conversa), idempotência (guarda em memória evita escalada duplicada — reforço no banco descartado, `handoffs.engagement_id` nunca foi populado), deploy automático (serviço Railway ligado ao GitHub, branch `main`, raiz `/agente-runtime`) — achado no caminho que **`main` nunca tinha recebido PR** desde a fase de pesquisa; PR #1 (aberto desde 08-10) squash-mergeado trazendo `main` em dia; desbloqueia o 031 (só falta o item solto `markRead:false`) |
